...
class RandomWeightedAverage(_Merge):
    ...
    def _merge_function(self, inputs):
        alpha = K.random_uniform((64, 56, 56, 3))
...
class WGANGP():
    ...
    def sample_images(self, epoch):
        ...
        dirs = "images_{}".format(str.zfill(str(int(epoch/5000)), 3))
        if not os.path.exists(dirs):
            os.makedirs(dirs)
        for i in range(500):
            generate_images = gen_imgs[i*9:i*9+9]
            output_fig(generate_images, file_name="images_{}".format(str.zfill(str(int(epoch/5000)), 3))
                                                    + "/{}_image".format(str.zfill(str(i), 3)))


if __name__ == '__main__':
    wgangp = WGANGP()
    wgangp.train(epochs=50000, batch_size=64, sample_interval=5000)
